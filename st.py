...
st.write("- Group sizes: ad =", n_ad, ", psa =", n_psa)
st.write(f"- Conversion rates: ad = {cr_ad:.4%}, psa = {cr_psa:.4%}")
st.write(f"- Difference (ad - psa) = {diff:.4%}")
st.write(f"- t-statistic = {t_stat:.4f}, p-value = {p_value:.5g}")
st.write(f"- 95% CI for difference = ({ci_lower:.4%}, {ci_upper:.4%})")

st.write(" ")
alpha = 0.05
rule= f"""Significance level: alpha = 0.05
- Decision rule:
